...
7. A range is not a figure. "128-141 kcal" and "3.4-3.7 g" state that sources disagree, and any single number taken from inside one is an estimate rather than a transcription — put such a line in `unreadable`, quoting the range, and never its midpoint.
8. If the photograph is not a product's own nutrition panel — a search-results page, an article, a recipe site, a summary of several products — transcribe nothing and say so in `unreadable`. Those figures describe a category, and the packet in the person's hand is not necessarily in it.
9. Copy each line verbatim into `as_printed`. The person will check your transcription against the packet in their hand, and that is only possible if they can see what you read.
